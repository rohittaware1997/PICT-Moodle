...
public static String uname,gname,assign_no,SubmissionDate,ScheduledDate,str1,sub,sch,marks,rollno,grpcode,subcode,approved,ass_name;
...
Marks =  Integer.parseInt(MARKS.getJSONObject("").getJSONArray("").getJSONObject(i).getString("Marks"));
    ...
    public void updatedb(int i) throws SQLException, ClassNotFoundException {
        ...
        conn = cd.getConnection();
        String q="";   
        q+= "update "+ass_name+" set Marks=? where (Rollno = ? and SubjectCode = (select SubjectCode from UserInfo where UserID=? and GroupCode=? ))";
   stmt=conn.prepareStatement(q);
    ...
    stmt.execute();
       ...
       q+= "update "+ass_name+" set Approved=? where (Rollno = ? and SubjectCode = (select SubjectCode from UserInfo where UserID=? and GroupCode=? ))";
       ...
       q+= "update "+ass_name+" set SubmissionDate=? where (Rollno = ? and SubjectCode = (select SubjectCode from UserInfo where UserID=? and GroupCode=? ))";
       ...
       q+= "update "+ass_name+" set ScheduledDate=? where (Rollno = ? and SubjectCode = (select SubjectCode from UserInfo where UserID=? and GroupCode=? ))";
